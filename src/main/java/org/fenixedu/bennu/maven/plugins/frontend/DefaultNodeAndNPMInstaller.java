...
public final class DefaultNodeAndNPMInstaller implements NodeAndNPMInstaller {
    ...
    DefaultNodeAndNPMInstaller(File workingDirectory, Platform platform, ArchiveExtractor archiveExtractor,
    ...
    public void install(String nodeVersion, String npmVersion) throws InstallationException {
        File nodeFolder = new File(workingDirectory.getAbsolutePath() + "/node");
    ...
    @Override
    public void install(String nodeVersion, String npmVersion, String nodeDownloadRoot, String npmDownloadRoot)
            ...
            try {
                ...
                if (nodeFile.exists()) {
                    final String version =
                            new NodeExecutor(workingDirectory, Arrays.asList("--version"), platform).executeAndGetResult();
            ...
            try {
                ...
                if (!nodeBinary.exists()) {
                    ...
                    if (!nodeBinary.renameTo(destination)) {
                        throw new InstallationException("Could not install Node: Was not allowed to rename " + nodeBinary
                                + " to " + destination);
                    }

                    if (!destination.setExecutable(true, false)) {
                        throw new InstallationException("Cound not install Node: Was not allowed to make " + destination
                                + " executable.");
                    }
